...
@testset "Known .ogg decoding" begin
    OggDecoder(joinpath(@__DIR__, "zero.ogg")) do dec
        ...
        @test bos(page)
        # the example file doesn't have the EOS flag set on the page, which decoders
        # should be able to handle, as Ogg should be resilient to truncated files
        @test !eos(page)
